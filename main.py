...
def jb_one_hot(aa_chain, aa_mapping, considered_special_aas=False):
    id_chain = []
    for aa in aa_chain:
        if (aa not in list(aa_mapping.keys())[:20]) and (considered_special_aas==False):
            print(aa, 'aa is in the chain and is not unconsidered.')
            break
        else:
            id_chain.append(aa_mapping[aa])
    return id_chain
...
if __name__ == "__main__":
    ...
    X_train = sequence.pad_sequences(encoded_docs, maxlen=maxlen ,padding='post', value=0.0)
    X_valid = sequence.pad_sequences(encoded_docs2, maxlen=maxlen ,padding='post', value=0.0)
    X_test = sequence.pad_sequences(encoded_docs3, maxlen=maxlen ,padding='post', value=0.0)
